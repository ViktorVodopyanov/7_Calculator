...
namespace Calculator.Services
{
    ...
    public class KafkaConsumerService : BackgroundService
    {
        ...
        private async Task StartConsumerLoop(CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    // крайние две цифры номера порта должен соответстввовать номеру варианта
                    await httpClient.PostAsJsonAsync($"http://localhost:5007/Calculator/Callback", inputData);
        ...
        {
            _kafkaConsumer.Close(); // Фиксация оффсетов и корректное выход из груп-пы.
